...
import { obtenerMisAgentes, editarAgente, eliminarAgente } from "../../services/conexion_api";
...
function ModalEditar({ agente, onClose, onGuardado }) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const actualizado = await editarAgente(agente.id, {
                nombre: form.nombre,
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-5">
                    {agente.estadoVerificacion === "APROBADO" && (
                        <div className="p-3 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-700 rounded-xl text-sm text-amber-700 dark:text-amber-400">
                            ...
                            Al guardar, el agente volverá a estado PENDIENTE para re-revisión del admin.
                        </div>
                    )}

                    <div>
                        <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-1.5">Nombre</label>
                    ...
                    <div>
                        <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-1.5">System Prompt</label>
                        <textarea name="systemPromt" value={form.systemPromt} onChange={handleChange} rows="4" required
                            className="w-full p-4 rounded-lg border border-slate-300 dark:border-slate-700 bg-slate-50 dark:bg-slate-900/50 font-mono text-sm focus:ring-2 focus:ring-purple-500/50 outline-none resize-y"></textarea>
                    </div>

                    <div>
                        <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-1.5">URL MCP (opcional)</label>
                        <input type="url" name="urlMcp" value={form.urlMcp} onChange={handleChange}
                            placeholder="https://tu-servidor-mcp.com"
                            className="w-full h-11 px-4 rounded-lg border border-slate-300 dark:border-slate-700 bg-transparent focus:ring-2 focus:ring-purple-500/50 outline-none" />
                    </div>

                    <div>
                        <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-1.5">Precio (USD)</label>
                        <input type="number" name="precio" value={form.precio} onChange={handleChange} min="0" required
                            className="w-full h-11 px-4 rounded-lg border border-slate-300 dark:border-slate-700 bg-transparent focus:ring-2 focus:ring-purple-500/50 outline-none" />
                    </div>
                    ...
                    <div className="flex justify-end gap-3 pt-2 border-t border-slate-200 dark:border-slate-800">
                        ...
                        <button type="submit" disabled={guardando}
                            ...
                            {guardando ? <><i className="fa-solid fa-circle-notch fa-spin mr-2"></i>Guardando...</> : "Guardar cambios"}
                        </button>
                    </div>
                </form>
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
            <div className="bg-white dark:bg-[#1a2230] rounded-2xl shadow-2xl w-full max-w-md border border-slate-200 dark:border-slate-700 p-6">
                ...
                <p className="text-sm text-slate-600 dark:text-slate-300 mb-5">
                    ¿Seguro que quieres eliminar <span className="font-bold">"{agente.nombre}"</span>?
                    Solo es posible si no tiene compras activas.
                </p>
                ...
                <div className="flex justify-end gap-3">
                    ...
                    <button onClick={handleBorrar} disabled={borrando}
                        ...
                        {borrando ? <><i className="fa-solid fa-circle-notch fa-spin mr-2"></i>Eliminando...</> : "Sí, eliminar"}
                    </button>
                </div>
            </div>
        </div>
...
export default function MisAgentes() {
    ...
    const handleGuardado = (actualizado) => {
        setMisagentes(prev => prev.map(a => a.id === actualizado.id ? actualizado : a));
            ...
            <main className="flex-1 max-w-7xl mx-auto px-6 py-12 w-full">
                <div className="mb-10 flex flex-col md:flex-row md:items-end justify-between gap-4">
                    <div>
                        ...
                        <p className="text-slate-500 dark:text-slate-400">
                            Gestiona el inventario de tus inteligencias artificiales activas en el marketplace.
                        </p>
                    </div>
                    <Link
                ...
                {loading ? (
                    ...
                    <div className="text-center py-16 bg-white dark:bg-[#1a2230] rounded-2xl border border-dashed border-slate-300 dark:border-slate-700">
                        ...
                        <h3 className="text-xl font-bold mb-2">Aún no tienes agentes publicados</h3>
                        <p className="text-slate-500 mb-6">
                            Sube tu primera inteligencia artificial al marketplace y empieza a generar ingresos.
                        </p>
                        <Link to="/desarrollador/subir-agente" className="text-purple-600 font-bold hover:underline">
                                ...
                                <div className="p-6 bg-slate-50 dark:bg-slate-800/20 flex-1">
                                    <div className="space-y-3 text-sm">
                                        ...
                                        <div className="flex justify-between">
                                            ...
                                            <span className="font-semibold">{agente.ventas ?? 0}</span>
                                        </div>
                                    </div>
                                </div>
